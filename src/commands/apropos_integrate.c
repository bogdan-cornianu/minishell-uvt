 ...
 * @return 0
 */
int apropos_default (int argcounter, char **argvector);
 ...
 */
int com_apropos (int argc, char **argv) {
	 ...
	 */
    if (argc == 2) {
        return apropos_default (argc, argv);
    }
...
int apropos_default (int argcounter, char **argvector)
{
	...
	if (manCmdFile == NULL)
	{
		fprintf (stderr, "Man file for command %s cannot be opened\n", argvector[1]);
		return 1;
	}
